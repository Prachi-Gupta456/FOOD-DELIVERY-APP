...
import { editFood } from "@/app/services/api";
...
export default function AddFood() {
    ...
    useEffect(() => {
        const item = shopData?.items?.find((item) => id === item._id)
        ...
        if (item) {
            ...
            setPrice(item.price)
            setFrontendImg(item.image)
            setBackendImg(item.image)
    ...
    const handleForm = async (e) => {
        ...
        const result = await editFood(form,id)
        console.log(result)
            ...
            <div className="max-w-lg w-full bg-white shadow-xl rounded-2xl p-8 border border-orange-100">
                ...
                <form onSubmit={handleForm} className="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="name" type="text" placeholder="Enter Food Name" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2
                    ...
                    <div>
                        ...
                        <select className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none cursor-pointer" value={foodType} onChange={(e) => setFoodType(e.target.value)}>
                    ...
                    <div>
                        ...
                        <select className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none cursor-pointer" value={category} onChange={(e) => setCategory(e.target.value)}>
                    ...
                    <div>
                        ...
                        <input id="price" type="number" placeholder="Enter Food Price" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full px-4 py-2
                    border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"/>
                    </div>
                    {/* ======== */}
                    ...
                    shadow-md  transition-all duration-200  ${pending ? "bg-gray-400 cursor-not-allowed" : "bg-[#ff4d2d] cursor-pointer text-white hover:bg-orange-600 hover:shadow-lg active:bg-gray-400"} `}>
                        {pending ? <ClipLoader size={20} color="white"/>: "Update"}
                    </button>
